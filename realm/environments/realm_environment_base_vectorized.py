...
class RealmEnvironmentBase:
    def __init__(
        ...
        if task_type in TASK_PROGRESSIONS:
            self.task_progression = [TASK_PROGRESSIONS[task_type] for _ in range(len(self.robots))]
        elif task in TASK_PROGRESSIONS:
            self.task_progression = [TASK_PROGRESSIONS[task] for _ in range(len(self.robots))]
        else:
    ...
    def apply_scene_fixes_from_cfg(self, og_vec_env, config_path, scene_model, scene_part):
        ...
        if scene_model in special_prims and scene_part in special_prims[scene_model]:
            #og.sim.stop()
            for env in og_vec_env.envs:
                for obj in env.scene.objects:
                    # TODO:
                    print(obj.name, obj.prim_path, obj._root_link_name)
    ...
    def update_robot_physics(self, og_vec_env, friction, armature):
        for env in og_vec_env.envs:
            robot = env.robots[0]
    ...
    def reset(self, og_vec_env, active_perturbations, supported_perturbations, config_path, scene_model, scene_part):
        obs, _ = og_vec_env.reset()
        self.apply_scene_fixes_from_cfg(og_vec_env, config_path, scene_model, scene_part)
        #self.disable_visual_toggles() # TODO: turn this back on

        self.was_lifted = False
        for i in range(len(self.task_progression)):
            for k in self.task_progression[i].keys():
                self.task_progression[i][k] = False
    ...
    def step(self, batched_action, og_vec_env, active_perturbations):
        obs, rew, terminated, truncated, info = og_vec_env.step(batched_action)
    ...
    def warmup(self, obs, og_vec_env, active_perturbations):
        ...
        for t in range(19):
            ...
            obs, rew, terminated, truncated, info = self.step(new_batched_action, og_vec_env, active_perturbations)
        ...
        if self.task_progression is not None:
            for i in range(len(obs)):
                ...
                for stage, is_completed_flag in self.task_progression[i].items():
                    ...
                    if is_completed_flag or all(checker_function(obs)):
