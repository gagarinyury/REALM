...
ПРОИСХОЖДЕНИЕ: файл взят целиком из ветки `port-to-og391` REALM (Martin Sedlacek, коммит
`dd091fd` от 10.08.2026) и не изменён. Оставлен как есть намеренно: наша собственная
реализация в `env_base.py::check_collisions` исходила из того, что величины импульсов в
3.9.1 недоступны — в коде так и было написано, — и потому считала столкновением каждый
контакт, включая покоящиеся. Это завышало счётчики: в прогонах 10.08.2026 на стоковом
роботе выходило до 25 столкновений со средой там, где рука ничего не задевала. Автор
показал, что магнитуды доступны — их можно прочитать из приватного вида RigidContactAPI, —
и восстановил прежний порог. Ниже — его исходный текст.
