...
if __name__ == "__main__":
    ...
    realm_env = RealmEnvironmentDynamic(
        ...
        perturbations=[],
        multi_view=False,
        # Рендер стенду не нужен -- нас интересуют только позы суставов, -- но с
        # no_rendering=True процесс умирает молча внутри конструктора окружения (13.08.2026,
        # OG 3.9.1 на Windows): лог обрывается на "app ready", ни исключения, ни трассировки.
        # Оставляем ту конфигурацию окружения, на которой в тот же день дважды прошёл полный
        # роллаут, чтобы стенд отличался от боевого прогона только отсутствием политики.
        no_rendering=False,
        rendering_mode="r",
        robot=args.robot,
    )
